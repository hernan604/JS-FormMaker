var FormMakerFieldMultiple = FormMakerFieldMultiple || ( function ( form_maker_instance ) {
    ...
    this.clone        = function () {
        return new FormMakerFieldMultiple( form_maker_instance );
    }
    ...
    this.create       = function ( field, value ) {
      ...
      console.log( _this.field_rendered.html() );
...
//  }
    ...
    this.append_actions = function ( div_multi ) {
        var div_action= $( "<div></div>" )
            .addClass( "action" )
            .appendTo( div_multi );
        var add_multi = $( "<div action='add'></div>" )
            .click( _this.clone_container )
            .append( $('<input/>').attr('type','button').val('Add') )
            .appendTo(div_action);
        var del_multi = $( "<div action='del'></div>" )
            .click( _this.del_container )
            .append( $( "<input/>" ).attr('type', 'button' ).val('Remove') )
            .appendTo(div_action);
    }
} );
